...
export default function LoginScreen() {
  ...
  const handleVerifyOTP = async () => {
    if (!otp.trim()) { showAlert('Code requis', 'Entrez le code reçu par email.'); return; }
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      >
        ...
        <View style={[styles.card, { backgroundColor: Colors.surface, ...Shadow }]}>
          {mode === 'otp' ? (
            <>
              <Text style={[styles.formTitle, { color: Colors.text }]}>Vérification email</Text>
              <Text style={[styles.formSubtitle, { color: Colors.textSubtle }]}>Un code a été envoyé à {email}</Text>
              <View style={{ marginBottom: Spacing.md }}>
                <Text style={[styles.fieldLabel, { color: Colors.textSubtle }]}>Code de vérification</Text>
                <TextInput style={inputStyle} placeholder="Ex: 1234" placeholderTextColor={Colors.textMuted} value={otp} onChangeText={setOtp} keyboardType="number-pad" maxLength={6} autoFocus />
              </View>
              <Pressable style={[styles.primaryBtn, { backgroundColor: Colors.primary }]} onPress={handleVerifyOTP} disabled={operationLoading}>
                {operationLoading ? <ActivityIndicator color="#fff" size="small" /> : <Text style={styles.primaryBtnText}>Confirmer et créer le compte</Text>}
              </Pressable>
              <Pressable onPress={() => setMode('register')} style={styles.linkBtn}>
                <Text style={[styles.linkText, { color: Colors.primary }]}>Renvoyer le code</Text>
              </Pressable>
            </>
          ) : (
            <>

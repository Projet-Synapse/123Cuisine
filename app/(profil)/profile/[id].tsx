...
import {
  ...
  getRecipesRatingStats,
  RecipeRatingStats,
} from '@/services/social/ratingService';
      ...
      {[1, 2, 3, 4, 5].map(star => (
        <Pressable key={star} onPress={() => !readonly && onRate?.(star)} hitSlop={4} disabled={readonly}>
          <MaterialIcons
            ...
            size={size}
            color={value !== null && value >= star ? starColor : emptyColor}
          />
        </Pressable>
      ))}
    </View>
...
export default function ProfileScreen() {
  ...
  const handleRate = async (rating: number) => {
    if (!user) {
      showAlert('Connexion requise', 'Notez les recettes après connexion.');
      return;
    }
    ...
    return (
      ...
      >
        <View style={[styles.recipeCardImg, { backgroundColor: Colors.surfaceMuted }]}>
          ...
          <View
            style={[
              ...
              {
                backgroundColor:
                  item.difficulty === 'Facile' ? '#4CAF5090' : item.difficulty === 'Moyen' ? '#F57C0090' : '#E5393590',
              },
            ]}
          >
        ...
        <View style={styles.recipeCardBody}>
          <Text style={[styles.recipeCardTitle, { color: Colors.text }]} numberOfLines={2}>
            {item.title}
          </Text>
        ...
        {!isOwnProfile ? (
          <Pressable
            style={[
              styles.followBtn,
              {
                backgroundColor: profile.isFollowing ? Colors.surfaceMuted : Colors.primary,
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              ...
              <Text style={[styles.emptyDesc, { color: Colors.textSubtle }]}>
                {isOwnProfile
                  ? 'Publiez une recette pour la partager avec la communauté.'

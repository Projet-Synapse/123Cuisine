...
export default function SearchScreen() {
  ...
  const handleFollowToggle = async (profile: UserProfile) => {
    if (!user) {
      showAlert('Connexion requise', 'Connectez-vous pour suivre des utilisateurs.');
      return;
    }
  ...
  const handleAddToList = (recipe: Recipe | PublicRecipe) => {
    ...
    showAlert('Ajouter à une liste', '', [
      ...shoppingLists.map(l => ({
        text: l.name,
        onPress: async () => {
          await addRecipeToList(l.id, recipe);
    ...
    >
      <View style={[styles.cardImage, { backgroundColor: Colors.surfaceMuted }]}>
        ...
        <Pressable style={styles.favBtn} onPress={() => void toggleFavorite(item.id)} hitSlop={8}>
          <MaterialIcons
            ...
            size={16}
            color={item.isFavorite ? Colors.favorite : '#fff'}
          />
        </Pressable>
      </View>
      <View style={styles.cardBody}>
        ...
        <View style={styles.metaRow}>
          ...
          <View style={[styles.diffBadge, { backgroundColor: Colors.difficulty[item.difficulty].bg }]}>
            <Text
              style={[
                styles.diffText,
                {
  ...
  const renderPublicRecipe = (item: PublicRecipe) => (
    <View
      key={item.id}
      style={[styles.publicCard, { width: publicCardWidth, backgroundColor: Colors.surface, ...Shadow.sm }]}
    >
      ...
      <View style={styles.publicBody}>
        <Pressable
          ...
          onPress={() => (item.authorId ? router.push(`/profile/${item.authorId}` as any) : null)}
        >
        ...
        <Text style={[styles.publicTitle, { color: Colors.text }]} numberOfLines={2}>
          {item.title}
        </Text>
        <View style={styles.metaRow}>
          <MaterialIcons name="schedule" size={12} color={Colors.textMuted} />
        ...
        <View style={styles.publicBtns}>
          <Pressable
            style={[
              styles.publicBtn,
              { borderColor: Colors.secondary + '40', backgroundColor: Colors.secondary + '08' },
            ]}
          ...
          <Pressable
            style={[styles.publicBtn, { borderColor: Colors.accent + '40', backgroundColor: Colors.accent + '08' }]}
            onPress={() => handleAddToPlaylist(item)}
          >
    ...
    <View style={[styles.userCard, { backgroundColor: Colors.surface, ...Shadow.sm }]}>
      ...
      >
        <View style={[styles.userAvatar, { backgroundColor: Colors.primary + '15' }]}>
          <Text style={{ fontSize: 22 }}>{(item.username || item.email)[0].toUpperCase()}</Text>
        </View>
        <View style={{ flex: 1 }}>
          <Text style={[styles.userName, { color: Colors.text }]}>{item.username || item.email.split('@')[0]}</Text>
          <Text style={[styles.userSub, { color: Colors.textSubtle }]}>
            {item.publicRecipeCount} recette{item.publicRecipeCount !== 1 ? 's' : ''}
          </Text>
          {item.username ? (
            <Text style={[styles.userSub, { color: Colors.textMuted, fontSize: 10 }]}>{item.email}</Text>
          ) : null}
        </View>
      </Pressable>
      <Pressable
        style={[
          styles.followBtn,
          {
            ...
            borderColor: item.isFollowing ? Colors.border : Colors.primary,
          },
        ]}
        ...
        {followLoading === item.id ? (
          ...
          <>
            <MaterialIcons
              ...
              size={14}
              color={item.isFollowing ? Colors.textSubtle : '#fff'}
            />
            <Text style={[styles.followBtnText, { color: item.isFollowing ? Colors.textSubtle : '#fff' }]}>
        ...
        <View style={[styles.tabBar, { backgroundColor: Colors.surfaceMuted }]}>
          {(
            ...
            <Pressable
              ...
              onPress={() => {
                setActiveTab(t.key);
                if (t.key === 'personnes' && search) void handleSearchUsers(search);
              }}
            >
              <MaterialIcons
                name={t.icon as any}
              ...
              {t.key === 'recettes' && recipes.length > 0 ? (
                <View
                  style={[
                    styles.countPill,
                    { backgroundColor: activeTab === t.key ? Colors.primary + '20' : Colors.border },
                  ]}
                >
                  <Text
                    style={[styles.countPillText, { color: activeTab === t.key ? Colors.primary : Colors.textMuted }]}
                  >
                    {recipes.length}
                  </Text>
                </View>
          ...
          >
            {(['Toutes', 'Facile', 'Moyen', 'Difficile'] as DifficultyFilter[]).map(d => (
              <Pressable
                ...
                style={[
                  styles.filterChip,
                  {
                    backgroundColor: difficultyFilter === d ? Colors.primary : Colors.surfaceMuted,
                    borderColor: difficultyFilter === d ? Colors.primary : Colors.border,
                  },
                ]}
                onPress={() => setDifficultyFilter(d)}
              >
                <Text style={[styles.filterChipText, { color: difficultyFilter === d ? '#fff' : Colors.textSubtle }]}>
                  {d}
                </Text>
              </Pressable>
            ))}
            <Pressable
              style={[
                styles.filterChip,
                {
                  backgroundColor: favoritesOnly ? Colors.favorite : Colors.surfaceMuted,
                  borderColor: favoritesOnly ? Colors.favorite : Colors.border,
                  flexDirection: 'row',
                  alignItems: 'center',
                  gap: 4,
                },
            ...
            >
              <MaterialIcons
                ...
                size={13}
                color={favoritesOnly ? '#fff' : Colors.textSubtle}
              />
              <Text style={[styles.filterChipText, { color: favoritesOnly ? '#fff' : Colors.textSubtle }]}>
                Favoris
              </Text>
            </Pressable>
            ...
            {[
              ...
              <Pressable
                ...
                style={[
                  styles.filterChip,
                  {
                    backgroundColor: sortBy === s.key ? Colors.accent : Colors.surfaceMuted,
                    borderColor: sortBy === s.key ? Colors.accent : Colors.border,
                    flexDirection: 'row',
                    alignItems: 'center',
                    gap: 4,
                  },
              ...
              >
                ...
                <Text style={[styles.filterChipText, { color: sortBy === s.key ? '#fff' : Colors.textSubtle }]}>
                  {s.label}
                </Text>
              </Pressable>
            ))}
            {hasActiveFilters ? (
              <Pressable
                style={[styles.filterChip, { backgroundColor: 'transparent', borderColor: Colors.border }]}
                onPress={() => {
        ...
        <ScrollView
          ...
          refreshControl={
            <RefreshControl
              ...
              onRefresh={() => void handleRefreshRecipes()}
        ...
        >
          <ScreenContainer style={{ padding: Spacing.md }}>
            <View style={styles.sectionRow}>
              ...
              <Text style={[styles.sectionCount, { color: Colors.textMuted, backgroundColor: Colors.surfaceMuted }]}>
            ...
            {filteredMyRecipes.length > 0 ? (
              ...
              <Text style={[styles.emptyHint, { color: Colors.textMuted }]}>
                {search || hasActiveDataFilters ? 'Aucun résultat pour ces filtres' : 'Aucune recette — créez-en une !'}
              </Text>
            )}
            {filteredPublic.length > 0 ? (
              <>
                <View style={[styles.sectionRow, { marginTop: Spacing.lg }]}>
                  ...
                  <Text
                    style={[styles.sectionCount, { color: Colors.textMuted, backgroundColor: Colors.surfaceMuted }]}
                  >
        ...
        <View style={{ flex: 1 }}>
          {!user ? (
            <View style={styles.centerState}>
              ...
              <Text style={[styles.emptyHint, { color: Colors.textSubtle }]}>
                pour rechercher et suivre des cuisiniers
              </Text>
              <Pressable
                style={[styles.emptyBtn, { backgroundColor: Colors.primary }]}
                ...
                ListHeaderComponent={
                  search.length < 2 ? (
                    <Text style={[styles.allUsersHeader, { color: Colors.textSubtle }]}>
                      {allUsers.length} cuisinier{allUsers.length !== 1 ? 's' : ''} inscrit
                      {allUsers.length !== 1 ? 's' : ''}
                    </Text>
                  ) : null
                }

...
export default function HomeScreen() {
  ...
  const t = useAppTheme();
  const { Colors, isDark } = t;
  const { FontSize } = t;
  const styles = useMemo(() => makeStyles(t), [t]);
  const { recipes, publicRecipes, shoppingLists, preferences, loading, addRecipe, refreshAll, refreshPublicRecipes } =
    useKitchen();
  const { user } = useAuth();
  ...
  const getPreferenceScore = useCallback(
    (recipe: Recipe | PublicRecipe): number => {
      let score = 0;
  ...
  const getSupermarket = (smId: string) =>
    SUPERMARKETS.find(s => s.id === smId) || SUPERMARKETS[SUPERMARKETS.length - 1];
      ...
      <ScreenContainer style={{ padding: Spacing.md }}>
          ...
          <View style={{ marginBottom: Spacing.lg }}>
            ...
            >
              {personalized.map(recipe => (
                <Pressable
                  key={recipe.id}
                  style={[styles.forYouCard, { backgroundColor: Colors.surface, ...Shadow.sm }]}
                  ...
                  <View style={[styles.forYouImage, { backgroundColor: Colors.surfaceMuted }]}>
                    ...
                    {recipe.isFavorite ? (
                      <View style={styles.favBadge}>
                        <MaterialIcons name="favorite" size={12} color={Colors.favorite} />
                      </View>
                    ) : null}
                  </View>
              ...
              <View>
                ...
                {followedUserIds.length > 0 ? (
                  <Text style={[styles.sectionHint, { color: Colors.textMuted }]}>
                    Recettes de vos abonnements en priorité
                  </Text>
                ) : null}
              </View>
              <Pressable onPress={() => router.push('/(tabs)/recipes')}>
                ...
                <View key={recipe.id} style={[styles.communityCard, { backgroundColor: Colors.surface, ...Shadow.sm }]}>
                  ...
                  <View style={{ padding: Spacing.sm }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4, marginBottom: 4 }}>
                      ...
                      <Text style={[styles.authorLabel, { color: Colors.primary }]} numberOfLines={1}>
                        {recipe.authorName}
                      </Text>
                    ...
                    <Text style={[styles.communityTitle, { color: Colors.text }]} numberOfLines={2}>
                      {recipe.title}
                    </Text>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 4 }}>
                      <MaterialIcons name="schedule" size={11} color={Colors.textMuted} />
            ...
            <View style={styles.sectionHeader}>
              ...
              <Pressable
                style={[
                  styles.generateBtn,
                  {
                    backgroundColor: aiLoading ? Colors.surfaceMuted : Colors.primary + '15',
              ...
              >
                ...
                <Text style={[styles.generateBtnText, { color: Colors.primary }]}>
                  {aiLoading ? '...' : aiLoaded ? 'Rafraîchir' : 'Générer'}
                </Text>
              </Pressable>
            </View>
              ...
              >
                {aiRecipes.map((recipe: any, idx: number) => (
                  <View
                    ...
                    style={[
                      styles.aiCard,
                      { backgroundColor: Colors.surface, borderColor: Colors.primary + '20', ...Shadow.sm },
                    ]}
                  >
                    ...
                    <Text style={[styles.aiCardTitle, { color: Colors.text }]} numberOfLines={2}>
                      {recipe.title}
                    </Text>
                    <Text style={[styles.aiCardDesc, { color: Colors.textSubtle }]} numberOfLines={2}>
                      {recipe.description}
                    </Text>
                    ...
                    <Pressable
                      ...
                      onPress={() => void handleSaveAIRecipe(recipe)}
                    >
        ...
        {activeList ? (
          <View style={{ marginBottom: Spacing.lg }}>
            ...
            <Pressable
              style={[
                styles.activeListCard,
                { backgroundColor: Colors.surface, borderLeftColor: activeList.color, ...Shadow.sm },
              ]}

...
export default function CreateRecipeScreen() {
  ...
  const addIngredient = () => {
    ...
    setIngredients(prev => [...prev, { id: Date.now().toString(), name: ingName.trim(), quantity: ingQty, unit: ingUnit, category: 'Légumes' }]);
  ...
  const handleSave = async () => {
    ...
    await addRecipe({ title: title.trim(), description: description.trim(), ingredients, steps: filledSteps, tags: selectedTags, duration: parseInt(duration) || 30, servings: parseInt(servings) || 4, difficulty, isFavorite: false, image: imageUrl, isPublic: isPublicRecipe });
    router.back();
  };
      ...
      <View style={{ flex: 1, backgroundColor: Colors.background, paddingTop: insets.top }}>
        ...
        <View style={[styles.header, { borderBottomColor: Colors.border, backgroundColor: Colors.surface }]}>
          ...
          <Pressable style={[styles.saveBtn, { backgroundColor: Colors.primary }]} onPress={() => void handleSave()} disabled={uploadingImage}>
            {uploadingImage ? <ActivityIndicator color="#fff" size="small" /> : <Text style={styles.saveBtnText}>Créer</Text>}
          </Pressable>
        </View>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: Spacing.md, paddingBottom: 60 }}>
          ...
          <View style={{ marginBottom: Spacing.lg }}>
            ...
            <Pressable style={[styles.photoPicker, { backgroundColor: Colors.surface, borderColor: Colors.border, ...Shadow }]} onPress={showImageOptions}>
              {localImageUri ? (
                <>
                  <Image source={{ uri: localImageUri }} style={StyleSheet.absoluteFillObject as any} contentFit="cover" />
                  ...
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ flex: 1 }}>
                    {UNITS.map(u => (
                      <Pressable key={u} style={[styles.unitChip, { backgroundColor: ingUnit === u ? Colors.primary : Colors.surfaceMuted, borderColor: ingUnit === u ? Colors.primary : Colors.border }]} onPress={() => setIngUnit(u)}>
                        <Text style={{ fontSize: FontSize.xs, color: ingUnit === u ? '#fff' : Colors.textSubtle, fontWeight: ingUnit === u ? FontWeight.semibold : FontWeight.regular }}>{u}</Text>
                      </Pressable>
                    ))}
                  </ScrollView>
                </View>
...
const styles = StyleSheet.create({
  ...
  stepNumBadge: { width: 28, height: 28, borderRadius: Radius.round, justifyContent: 'center', alignItems: 'center', flexShrink: 0, marginTop: 8 },

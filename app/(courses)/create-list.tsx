...
export default function CreateListScreen() {
  ...
  const handleSave = async () => {
    if (!name.trim()) {
      showAlert('Champ requis', 'Donnez un nom à votre liste.');
      return;
    }
    await addShoppingList({
      ...
      items,
      color: selectedSupermarket.color,
    });
    router.back();
  };
                  ...
                  <Pressable
                    ...
                    style={[
                      styles.smCard,
                      {
                        ...
                        borderColor: selectedSupermarket.id === sm.id ? sm.color : Colors.border,
                        borderWidth: selectedSupermarket.id === sm.id ? 2 : 1,
                        ...Shadow.sm,
                      },
                    ]}
                  ...
                  >
                    ...
                    <Text
                      style={[
                        styles.smName,
                        {
                          color: selectedSupermarket.id === sm.id ? sm.color : Colors.textSubtle,
                          fontWeight: selectedSupermarket.id === sm.id ? FontWeight.bold : FontWeight.medium,
                        },
                      ]}
                    >
                      {sm.name}
                    </Text>
                    {selectedSupermarket.id === sm.id ? (
                      <MaterialIcons name="check-circle" size={16} color={sm.color} />
                    ) : null}
                  </Pressable>
                ))}
              </ScrollView>
            ...
            <View style={{ marginBottom: Spacing.lg }}>
              ...
              <View style={[styles.card, { backgroundColor: Colors.surface, ...Shadow.sm }]}>
                ...
                <TextInput
                  ...
                  placeholder={"Nom de l'article (ex: jus d'orange...)"}
                  placeholderTextColor={Colors.textMuted}
                  value={itemName}
                  ...
                  >
                    {searchLoading ? (
                      <View style={styles.searchLoading}>
                        <ActivityIndicator size="small" color={selectedSupermarket.color} />
                      </View>
                        ...
                        {searchResults.map(p => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <Text style={[styles.searchRowPrice, { color: selectedSupermarket.color }]}>
                                ~€{estPrice.toFixed(2)}
                              </Text>
                    ...
                    {UNITS.map(u => (
                      <Pressable
                        ...
                        style={[
                          styles.unitChip,
                          {
                            backgroundColor: itemUnit === u ? selectedSupermarket.color : Colors.surfaceMuted,
                            borderColor: itemUnit === u ? selectedSupermarket.color : Colors.border,
                          },
                        ]}
                        onPress={() => setItemUnit(u)}
                      >
                        <Text style={[styles.unitText, { color: itemUnit === u ? '#fff' : Colors.textSubtle }]}>
                          {u}
                        </Text>
                      </Pressable>
                    ))}
                  ...
                  {INGREDIENT_CATEGORIES.slice(0, 8).map(cat => (
                    <Pressable
                      ...
                      style={[
                        ...
                        {
                          backgroundColor:
                            itemCategory === cat ? selectedSupermarket.color + '20' : Colors.surfaceMuted,
                          borderColor: itemCategory === cat ? selectedSupermarket.color : Colors.border,
                        },
                      ]}
                    ...
                    >
                      <Text
                        style={[
                          styles.catText,
                          { color: itemCategory === cat ? selectedSupermarket.color : Colors.textSubtle },
                        ]}
                      >
                        {cat}
                      </Text>
                ...
                <Pressable
                  style={[styles.addItemBtn, { backgroundColor: selectedSupermarket.color }]}
                  onPress={addItem}
                >
